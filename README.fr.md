...
- **Inventaire des variantes** — bande passante, résolution, frame rate et codecs de chaque variante d'une master playlist.
- **Contrôles de conformité** — durées de segment dépassant `EXT-X-TARGETDURATION` (RFC 8216 §4.3.3.1), attributs `CODECS`/`RESOLUTION` manquants, bandes passantes en double, comptage des discontinuités, dérive entre `EXT-X-PROGRAM-DATE-TIME` et la somme des durées de segments.
- **Monitoring live** (`--live`) — interroge une media playlist live à la moitié de la target duration, compte les rafraîchissements avec et sans progression, et estime le retard du bord de playlist par rapport au temps réel via les tags PDT.
...
- Contrôle des segments (disponibilité, taille vs durée déclarée, sniffing TS/fMP4)
- Low-latency HLS (segments partiels, preload hints)
